...
interface IRentTable {
	rents: IRent[];
	handleRejectRent: (rentId: number, editedRent: IEditRentDto) => Promise<void>;
}

export const AcceptedRentTable = ({ rents, handleRejectRent }: IRentTable) => {
	return (
					...
					<tr key={rent.id} data-cy="car-table">
						...
						<td className="relative whitespace-nowrap py-4 pl-4 pr-4 text-right text-sm font-medium sm:pr-0">
							<button
								...
								onClick={() => handleRejectRent(rent.id, { rejected: true })}
							>
								Reject
							</button>
						</td>
